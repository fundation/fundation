    ...
    servers.forEach(function(server){
      ...
      var connection = new Promise(function(resolve, reject){
        ...
        if ( server.username && server.password ) {
          ...
          authDebug = server.username + ':<hidden password>' + '@';
        }

        var port = (server.port) ? server.port : 27017;

        debug('Mongodb: ' + server.name + ' (' + authDebug + server.host + ':' + port + '/' + server.database + ')');
        MongoClient.connect('mongodb://' + auth + server.host + ':' + port + '/' + server.database + '', function(error, database) {
          if ( error ) {
            return reject(error);
          }
